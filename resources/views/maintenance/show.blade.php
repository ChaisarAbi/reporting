    ...
    <div class="mb-8">
        <div class="flex justify-between items-center">
            ...
            <div class="flex gap-2">
                <a href="{{ route('maintenance.reports') }}" class="inline-flex items-center px-6 py-3 border border-navy-300 text-sm font-medium rounded-xl text-navy-700 bg-white hover:bg-navy-50 hover:border-navy-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 transition-all duration-200 shadow-lg">
                    <svg class="-ml-1 mr-2 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                    </svg>
                    Kembali ke Daftar
                </a>
                <a href="{{ route('maintenance.reports.export.single.pdf', $breakdownReport) }}" 
                   class="inline-flex items-center px-6 py-3 border border-transparent text-sm font-medium rounded-xl shadow-lg text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-all duration-200">
                    <svg class="-ml-1 mr-2 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @if($breakdownReport->status === 'new')
                    <form action="{{ route('maintenance.start-repair', $breakdownReport) }}" method="POST">
                        @csrf
                        <button type="submit" class="inline-flex items-center px-6 py-3 border border-transparent text-sm font-medium rounded-xl shadow-lg text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200">
                            Mulai Perbaikan
                        </button>
                    </form>
                @endif
                @if($breakdownReport->status === 'in_progress')
                    <a href="{{ route('maintenance.show-complete-form', $breakdownReport) }}" class="inline-flex items-center px-6 py-3 border border-transparent text-sm font-medium rounded-xl shadow-lg text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200">
    ...
    <div class="bg-white shadow-2xl rounded-2xl mb-8 border border-navy-200">
        <div class="px-6 py-6 sm:p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-navy-50 rounded-xl p-4 border border-navy-200">
                    <label class="block text-sm font-medium text-navy-600">Pelapor</label>
                    <p class="mt-2 text-base text-navy-900 font-medium">{{ $breakdownReport->reporter_name ?? $breakdownReport->reporter->name }}</p>
                </div>
                <div class="bg-navy-50 rounded-xl p-4 border border-navy-200">
    ...
    <div class="bg-white shadow-2xl rounded-2xl mb-8 border border-navy-200">
        <div class="px-6 py-6 sm:p-8">
            ...
            <div>
                ...
                <div class="bg-navy-50 rounded-xl p-6 border border-navy-200">
                    <p class="text-base text-navy-900 whitespace-pre-line leading-relaxed">{{ $breakdownReport->technician_notes }}</p>
                </div>
            </div>
            @endif
        </div>
